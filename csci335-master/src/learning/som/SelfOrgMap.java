...
public class SelfOrgMap<V> {
    ...
    public SOMPoint bestFor(V example) {
        SOMPoint s = new SOMPoint(getMapHeight(), getMapWidth());
        for(int i = 0; i < getMapWidth(); i++) {
            for(int j = 0; j < getMapHeight(); j++) {
                double d1 = distance.applyAsDouble(getNode(i,j), example);
                double d2 = s.distanceTo(i,j);
                if(d2 < d1) {
                    SOMPoint s2 = new SOMPoint(i,j);
                    s.equals(s2);
                }
            }
         }
