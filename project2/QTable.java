...
public class QTable {
    ...
    public int getBestAction(int state) {
        int highest = 0;
        double h = q[state][0];
        for(int i = 0; i <= q[state].length-1; i++) {
            if(q[state][i] > h) {
                h = q[state][i];
                highest = i;
            }
    ...
    public int leastVisitedAction(int state) {
        int least = 0;
        int l = visits[state][0];
        for(int i = 0; i <= visits[state].length-1; i++) {
                if(visits[state][i] < l) {
                    l = visits[state][i];
                    least = i;
                }
    ...
    public int senseActLearn(int newState, double reward) {
        ...
        int a;
        q[lastState][lastAction] = (1 - lr) * q[lastState][lastAction] + lr * ((discount * q[newState][getBestAction(newState)]) +  reward);
        visits[lastState][lastAction] += 1;
        if(isExploring(newState)) {
            a = leastVisitedAction(newState);
        }
        else {
            a = getBestAction(newState);
        }
        lastState = newState;
        lastAction = a;
        return a;
